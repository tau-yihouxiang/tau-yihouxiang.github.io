    ...
    req.on('end', () => {
      ...
      try {
        ...
        for (const entry of entries) {
          const time = entry.ts ? new Date(entry.ts).toISOString() : new Date().toISOString();
          const level = entry.level || 'INFO';
          const msg = entry.msg || '';
          const { ts, level: _l, msg: _m, ...rest } = entry;
          const extra = Object.keys(rest).length ? ' ' + JSON.stringify(rest) : '';
          console.log(`[CLIENT ${time}] [${level}] [${clientIP}] ${msg}${extra}`);
        }
      } catch { console.log(`[CLIENT RAW] [${clientIP}]`, body); }
